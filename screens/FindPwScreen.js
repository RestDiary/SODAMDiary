...
function FindPwScreen({ navigation }) {
  ...
  const checkNum = () => {
    ...
    axios.post('http://people-env.eba-35362bbh.ap-northeast-2.elasticbeanstalk.com:3001/checkNum', null, {
      params: { 
        id: id,
        email: email
      }
      ...
      .then(res => {
        if(res.data !== 1) { // 1을 받아오면 없는 아이디 또는 이메일
        //   clearTimeout(timer);
        ...
        <View style={{ ...styles.container, backgroundColor:nowTheme.bg }}>
          ...
          <View style={{...styles.inputLayout}}>
            ...
            <View style={{...styles.inputCheck}}>
              ...
              <Text style={{color:"white"}}>{minute}{div}{second}</Text>
            </View>
