...
function JoinScreen({ navigation }) {
  ...
  const register = async() => {
    ...
    await axios.post('http://people-env.eba-35362bbh.ap-northeast-2.elasticbeanstalk.com:3001/register', null, {
      params: {
        id: id,
        email: email,
        pw: pw
    ...
    <View style={{ ...styles.container, backgroundColor:"#071D3A" }}>
      ...
      <View style={{...styles.inputLayout}}>
        ...
        <View style={{...styles.inputCheck}}>
          <Text style={checkId ? {color:"#32CD99"} :{color:"#ff0000"} }>{changeId}</Text>
        </View>
        {/* 이메일 박스 */}
        <TextInput style={{...styles.inputBox}} 
        placeholder="이메일 입력"
        ...
        {/* margin */}
        <View style={{...styles.inputCheck}}>
            <Text style={{color:"#32CD99"}}>비밀번호는 4~15글자 사이의 문자만 사용할 수 있습니다.</Text>
        </View>
